...
GuiImageData::GuiImageData(gdImagePtr gdImg)
{
    texture = NULL;
    sampler = NULL;
	loadGDImage(gdImg);
...
void GuiImageData::loadBufImage()
{
    texture->surface.mip_data = NULL;
    memcpy(texture->surface.image_data, renderTexture, sizeof(u32) * (256 * 128));
    GX2Invalidate(GX2_INVALIDATE_CPU_TEXTURE, texture->surface.image_data, texture->surface.image_size);
    sampler = new GX2Sampler;
    GX2InitSampler(sampler, GX2_TEX_CLAMP_CLAMP, GX2_TEX_XY_FILTER_BILINEAR);
}

void GuiImageData::loadGDImage(gdImagePtr gdImg)
{
    texture->surface.mip_data = NULL;

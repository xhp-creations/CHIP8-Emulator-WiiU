    ...
    , exitCode(EXIT_RELAUNCH_ON_LOAD)
{
    ...
    //! load resources
    Resources::LoadFiles("sd:/wiiu/apps/CHIP8/resources");
    ...
    //! create buzzer sound
    squareWave = new GuiSound(Resources::GetFile("squarewave.ogg"), Resources::GetFileSize("squarewave.ogg"));
    squareWave->SetLoop(false);
    squareWave->Play();
    squareWave->SetVolume(80);
...
void Application::playBuzzer()
{
    log_printf("Playing Buzz\n");
    squareWave->Play();
}
...
void Application::executeThread(void)
{
	...
	while(!exitApplication)
	{
        ...
        if (emulateROM)  // when emulateROM is true, we can run the emulation function each cycle
        {
            bool playBuzz = mainWindow->RunEmu(controller[controlUsed]); // call to run the emulator and setup to read the controller set by variable controlUsed
            
            if (playBuzz && !buzzPlaying)
            {
                buzzPlaying = true;
                log_printf("Play Buzz\n");
                Application::playBuzzer();
            }
            if (!playBuzz && buzzPlaying)
            {
                buzzPlaying = false;
                log_printf("Stop Buzz\n");
            }
        }
